...
for (var ch = 0; ch <= 0xF; ++ch) {
    ...
    XoneK2.controllers[ch].leftEncoderUsed = false;
...
XoneK2.decksBottomLeftEncoderPress = function (channel, control, value, status) {
    ...
    if(!XoneK2.controllers[channel].isShifted) {
        if (XoneK2.controllers[channel].leftEncoderIsPressed) {
            XoneK2.controllers[channel].leftEncoderUsed = -1;
        } else if (XoneK2.controllers[channel].leftEncoderUsed == -1) {
            engine.setValue("[PreviewDeck1]", "play", 
                            !engine.getValue("[PreviewDeck1]", "play"));
            XoneK2.controllers[channel].leftEncoderUsed = 0;
        }
    }
...
XoneK2.decksBottomLeftEncoder = function (channel, control, value, status) {
    if (!XoneK2.controllers[channel].isShifted) {
        if (!XoneK2.controllers[channel].leftEncoderIsPressed) {
            var currentValue = engine.getParameter("[PreviewDeck1]", "playposition");
            ...
            //  mul = 0.0001;
            //}
            engine.setValue("[PreviewDeck1]", "playposition", Math.max(0.0, Math.min(1.0, currentValue + dir * mul)));
        } else {
            XoneK2.controllers[channel].leftEncoderUsed = 1;
